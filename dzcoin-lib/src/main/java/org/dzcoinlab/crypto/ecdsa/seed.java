...
public class seed {
    ...
    public static ikeypair createkeypair(byte[] seedbytes, int accountnumber) {
        if(seedbytes.length != 16)
            throw new dzcoinexception("seedbytes must be 16 bytes: " + hex.tohexstring(seedbytes));
    ...
    public static ikeypair createkeypair(byte[] seedbytes) {
        if(seedbytes.length != 16)
            throw new dzcoinexception("seedbytes must be 16 bytes: " + convert.bytestohex(seedbytes));
    ...
    public static byte[] genseedfrombtcpriv(byte[] priv) {
        if(priv.length != 32) throw new dzcoinexception("btc private key must be 32 bytes!");
