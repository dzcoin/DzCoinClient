...
import org.dzcoinlab.btc.account.address;
...
import static org.dzcoinlab.btc.bitutil.bytestohexstring;
import static org.dzcoinlab.btc.bitutil.parseashexorbase58;
...
public class eckey implements serializable {
    ...
    */
     ...
     */
    public static eckey getecfromprivstr(string privstr) throws dzcoinexception {
        if(privstr == null || privstr.length() < 32) throw new dzcoinexception("wrong private key : " + privstr);
        byte[] priv;
        ...
        if(len == 44 && privstr.endswith("=")) {//maybe base64
            ...
            if(src.length != 37 || src[0] != (byte)0x80)
                throw new dzcoinexception("wrong private key of btc-qt format: " + bytestohexstring(src));
            priv = new byte[32];
            system.arraycopy(src, 1, priv, 0, 32);
        } else if(len == 64) {          //maybe hex
            priv = parseashexorbase58(privstr);
        } else {
            throw new dzcoinexception("unknown private key format: " + privstr);
        }
    ...
    /**
     * exports the private key in the form used by the satoshi com.dzcoin.client "dumpprivkey" and "importprivkey" commands. use
