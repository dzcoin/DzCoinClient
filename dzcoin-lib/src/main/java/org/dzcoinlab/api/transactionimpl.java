package org.dzcoinlab.api;

import com.google.gson.gson;

import org.dzcoinlab.client.ws.dzcoinwebsocketclient;
import org.dzcoinlab.core.*;
import org.dzcoinlab.core.fields.field;
import org.dzcoinlab.core.hash.b58;
import org.dzcoinlab.core.serialized.enums.ledgerentrytype;
import org.dzcoinlab.core.types.known.tx.transaction;
import org.dzcoinlab.core.types.known.tx.result.affectednode;
import org.dzcoinlab.core.types.known.tx.result.transactionmeta;
import org.dzcoinlab.core.types.known.tx.signed.signedtransaction;
import org.dzcoinlab.core.types.known.tx.txns.*;
import org.dzcoinlab.core.uint.uint32;
import org.dzcoinlab.crypto.ecdsa.ikeypair;
import org.dzcoinlab.crypto.ecdsa.seed;
...
/**
 * implements of all kinds of transactions, defined by websocket api of dzcoin.
 * @see  "api documents"
 * @see org.dzcoinlab.test.testwebsocket for usage
 */
...
//        data.put("source_currencies", collections.singletonlist(currency));
        ...
        string json = dzcoinwebsocketclient.req(postdata);
    ...
    public jsonobject bookoffers(string currency1, string issuer1, string currency2, string issuer2, string taker, int limit) throws apiexception {
        ...
        string postdata = new gson().tojson(data);
        string jsonstrask = dzcoinwebsocketclient.req(postdata);
        if (jsonstrask != null) {
        ...
        string jsonstrbids = dzcoinwebsocketclient.req(postdata);
        if (jsonstrbids != null) {
    ...
    public string accountoffers(string address, int limit, string c1, string issuer1, string c2, string issuer2, string marker) throws apiexception {
        ...
        string postdata = new gson().tojson(data);
        string jsonresult = dzcoinwebsocketclient.req(postdata);
    ...
    /**
     * all methods using this maketx, will call submit interface of "dzcoind", and blob data needs be signed.
     * see: https://ripple.com/build/rippled-apis/#submit
     */
            ...
            if (meta.engineresult().asinteger() != 0) {
                ...
                if (tx instanceof offercreate) {
                    ...
                    if (address.equals(offercreate.takergets().issuerstring()) || address.equals(offercreate.takerpays().issuerstring())) {
                        item.settype("offer_dzcoining");
                    } else
                        item.settype("offercreate");
                    if (item.getsender().equals(address) || item.gettype().equals("offer_dzcoining")) {
