...
public class wallet {
    ...
    public static wallet fromseedstring(string seed) {
        if (seed == null || seed.length() < b58.len_family_seed - 2 || !seed.startswith("s"))
            throw new dzcoinexception("accountid.fromseedstring() param error: " + seed);
        byte[] b16 = b58.decodefamilyseed(seed);
        if (b16.length != 16) throw new dzcoinexception("decodefamilyseed() not 16 bytes: " + hex.tohexstring(b16));
