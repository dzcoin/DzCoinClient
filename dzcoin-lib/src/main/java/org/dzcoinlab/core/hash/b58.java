package org.dzcoinlab.core.hash;

import org.dzcoinlab.core.utils;
...
public class b58 {
    ...
    public byte[] decode(string input) throws dzcoinexception {
        if (input.length() == 0) {
            return new byte[0];
        }
        ...
        for (int i = 0; i < input.length(); ++i) {
            ...
            if (digit58 < 0) {
                throw new dzcoinexception("illegal character " + c + " at " + i);
            }
    ...
    public biginteger decodetobiginteger(string input) throws dzcoinexception {
     ...
     *
     * @throws org.dzcoinlab.core.exception.dzcoinexception if the input is not basefields 58 or the checksum does not validate.
     */
    public byte[] decodechecked(string input, int version) throws dzcoinexception {
        byte buffer[] = decode(input);
        if (buffer.length < 4)
            throw new dzcoinexception("input too short");
        byte actualversion = buffer[0];
        if (actualversion != version) {
            throw new dzcoinexception("bro, version is wrong yo:" + input + " ver=" + actualversion + " need=" + version);
        }
        ...
        if (!arrays.equals(checksum, hashed))
            throw new dzcoinexception("checksum does not validate");
    ...
    /** family seed */
    public byte[] decodefamilyseed(string seed) throws dzcoinexception {
    ...
    /** addrress */
    public byte[] decodeaddress(string address) throws dzcoinexception {
    ...
    /** node public (by fau) */
    public byte[] decodenodepublic(string np) throws dzcoinexception {
    ...
    public byte[] decodeaccountpublic(string ap) throws dzcoinexception {
